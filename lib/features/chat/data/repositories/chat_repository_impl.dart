...
class ChatRepositoryImpl extends FirestoreRepository<ChatRoom>
  ...
  Future<ChatRoom> createOrGetDirectChat(
    ...
    try {
      ...
      LoggerService.error('Failed to create or get direct chat',
          tag: _tag, error: e);
      rethrow;
    }
  }
  ...
  Future<String> sendMessage({
    ...
    try {
      ...
      LoggerService.info(
          'Message sent to chat room $chatRoomId with attachment: ${attachmentUrl != null}',
          tag: _tag);
      return ref.id;
    } catch (e) {
  ...
  Future<void> deleteMessage(String chatRoomId, String messageId) async {
    try {
      ...
      LoggerService.info(
          'Message $messageId deleted from chat room $chatRoomId',
          tag: _tag);
    } catch (e) {
  ...
  Future<void> editMessage(
    ...
    try {
      ...
      LoggerService.info('Message $messageId edited in chat room $chatRoomId',
          tag: _tag);
    } catch (e) {
  ...
  Future<void> addParticipant(
      String chatRoomId, String userId, ParticipantInfo participantInfo) async {
    try {
      ...
      LoggerService.info('Participant $userId added to chat room $chatRoomId',
          tag: _tag);
    } catch (e) {
  ...
  Future<void> removeParticipant(String chatRoomId, String userId) async {
    try {
      ...
      LoggerService.info(
          'Participant $userId removed from chat room $chatRoomId',
          tag: _tag);
    } catch (e) {
  ...
  Future<void> markMessagesAsRead(String chatRoomId) async {
    try {
      ...
      LoggerService.info('Marked ${unreadMessages.length} messages as read',
          tag: _tag);
    } catch (e) {
      LoggerService.error('Failed to mark messages as read',
          tag: _tag, error: e);
      rethrow;
    }
  }
  ...
  Future<void> markMessageAsDelivered(
    ...
    try {
      ...
      LoggerService.error('Failed to mark message as delivered',
          tag: _tag, error: e);
      rethrow;
    }
  }
  ...
  Future<List<ChatRoom>> searchChatRooms(String query) async {
    try {
      final chatRooms = await list(
        conditions: [
          QueryCondition(
              field: 'participantIds', arrayContains: _currentUserId),
        ],
      );

      final lowercaseQuery = query.toLowerCase();
      return chatRooms
          .where((room) => room.name.toLowerCase().contains(lowercaseQuery))
          .toList();
    } catch (e) {
    ...
    try {
      ...
      for (final room in chatRooms) {
        if (room.unreadCounts != null &&
            room.unreadCounts![_currentUserId] != null) {
      ...
      LoggerService.error('Failed to get all unread counts',
          tag: _tag, error: e);
      return {};
    }
  }
  ...
  Future<void> batchDeleteMessages(
    ...
    try {
      ...
      await batch.commit();
      LoggerService.info('Batch deleted ${messageIds.length} messages',
          tag: _tag);
    } catch (e) {
      LoggerService.error('Failed to batch delete messages',
          tag: _tag, error: e);
      rethrow;
    }
  }
  ...
  Future<void> batchUpdateMessageStatus(
      String chatRoomId, List<String> messageIds, MessageStatus status) async {
    try {
      final batch = _firestore.batch();
      ...
      await batch.commit();
      LoggerService.info(
          'Batch updated ${messageIds.length} messages to status ${status.name}',
          tag: _tag);
    } catch (e) {
      LoggerService.error('Failed to batch update message status',
          tag: _tag, error: e);
      rethrow;
    }
  }
