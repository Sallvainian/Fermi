...
class NotificationService {
    ...
    try {
      if (kIsWeb) {
        ...
        if (permission == 'granted') {
          LoggerService.info(
            'Web notification permission already granted',
            tag: 'NotificationService',
          );
          return true;
        } else if (permission == 'denied') {
          LoggerService.warning(
            'Web notification permission denied by user',
            tag: 'NotificationService',
          );
    ...
    if (kIsWeb) {
      try {
        ...
        if (platform.WebNotification.supported) {
          ...
          LoggerService.info(
            'Web notification permission status: $permission',
            tag: 'NotificationService',
          );
          return permission;
        } else {
        ...
        LoggerService.error(
          'Error checking web notification permission',
          error: e,
          tag: 'NotificationService',
        );
        return 'denied';
  ...
  Future<void> scheduleEventReminder(CalendarEvent event) async {
    LoggerService.info(
      'Event reminder scheduled: ${event.title}',
      tag: 'NotificationService',
    );
  ...
  Future<void> scheduleAssignmentReminder(Assignment assignment) async {
    LoggerService.info(
      'Assignment reminder scheduled: ${assignment.title}',
      tag: 'NotificationService',
    );
    // Store notification record only
    await _storeNotificationRecord(
  ...
  Future<void> sendImmediateNotification({
    ...
    LoggerService.info(
      'Immediate notification: $title - $body',
      tag: 'NotificationService',
    );
    // Store notification record only
    await _storeNotificationRecord(
    ...
    if (response.payload != null) {
      ...
      try {
        ...
        if (type == 'assignment' && id != null) {
          ...
          LoggerService.info(
            'Assignment notification tapped: $id',
            tag: 'NotificationService',
          );
        } else if (type == 'message' && id != null) {
          // Navigate to message/chat
          LoggerService.info(
            'Message notification tapped: $id',
            tag: 'NotificationService',
          );
        }
  ...
  Future<bool> retryPermissionRequest() async {
    if (_notificationPermissionDenied) {
      LoggerService.info(
        'Retrying notification permission request',
        tag: 'NotificationService',
      );
