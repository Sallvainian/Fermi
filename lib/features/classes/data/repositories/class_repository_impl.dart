...
class ClassRepositoryImpl extends FirestoreRepository<ClassModel>
  ...
  Future<void> addStudent(String classId, String studentId) async {
    try {
      ...
      LoggerService.info('Added student $studentId to class $classId',
          tag: tag);
    } catch (e) {
  ...
  Future<void> removeStudent(String classId, String studentId) async {
    try {
      ...
      LoggerService.info('Removed student $studentId from class $classId',
          tag: tag);
    } catch (e) {
      LoggerService.error('Failed to remove student from class',
          tag: tag, error: e);
      rethrow;
    }
  }
  ...
  Future<List<ClassModel>> getActiveClasses(
      String teacherId, String academicYear) async {
    return await list(
      conditions: [
        QueryCondition(field: 'teacherId', isEqualTo: teacherId),
  ...
  @override
  Future<void> enrollMultipleStudents(
    ...
    try {
      ...
      await batch.commit();
      LoggerService.info(
          'Enrolled ${studentIds.length} students in class $classId',
          tag: tag);
    } catch (e) {
      LoggerService.error('Failed to enroll multiple students',
          tag: tag, error: e);
      rethrow;
    }
  }
  ...
  Future<ClassModel?> getClassByEnrollmentCode(String enrollmentCode) async {
    try {
      ...
      LoggerService.error('Failed to get class by enrollment code',
          tag: tag, error: e);
      rethrow;
    }
  }
  ...
  /// @throws Exception if enrollment fails
  @override
  Future<ClassModel> enrollWithCode(
    ...
    try {
      ...
      LoggerService.info(
          'Enrolled student $studentId in class ${classModel.name} via code',
          tag: tag);
      return updatedClass;
    } catch (e) {
  ...
  Future<String> regenerateEnrollmentCode(String classId) async {
    try {
      const chars =
          'ABCDEFGHJKLMNPQRSTUVWXYZ23456789'; // Exclude ambiguous characters
      const codeLength = 6;
      const maxAttempts = 100;
      ...
      LoggerService.info('Generated new enrollment code for class $classId',
          tag: tag);
      return newCode;
    } catch (e) {
      LoggerService.error('Failed to regenerate enrollment code',
          tag: tag, error: e);
      rethrow;
    }
  }
