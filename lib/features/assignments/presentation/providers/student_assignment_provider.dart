...
class StudentAssignmentProvider with ChangeNotifier {
  ...
  final StreamController<List<StudentAssignment>>
      _studentAssignmentsController =
  ...
  Future<void> setupAssignmentsStream() async {
    ...
    try {
      ...
      if (student != null) {
        ...
        debugPrint(
            '[StudentAssignmentProvider] Student name: ${student.displayName}');
  ...
  Stream<List<StudentAssignment>> _createCombinedStream(List<String> classIds) {
    ...
    final assignmentsSubscription = _assignmentRepository
      ...
      (assignments) {
        ...
        latestAssignments = assignments.where((assignment) {
          ...
          return assignment.isPublished &&
              ...
              (assignment.publishAt == null ||
                  assignment.publishAt!.isBefore(now));
        }).toList();
        combineAndEmit();
      },
      onError: (error) {
    ...
    final submissionsSubscription =
        _submissionRepository.getStudentSubmissions(_currentStudentId!).listen(
      (submissions) {
    ...
    final gradesSubscription =
        _gradeRepository.getStudentGrades(_currentStudentId!).listen(
      (grades) {
    ...
    try {
      ...
      if (file != null && file.bytes != null) {
        ...
        final fileRef = storageRef
            .child('submissions/$assignmentId/$_currentStudentId/$fileName');
      ...
      // Update local state
      final index =
          _assignments.indexWhere((a) => a.assignment.id == assignmentId);
      if (index != -1) {
  ...
  List<StudentAssignment> getAssignmentsByCategory(String category) {
    return _assignments
        .where((a) => a.assignment.category == category)
        .toList();
  }
  ...
  Map<String, dynamic> getStatistics() {
    ...
    return {
      ...
      'letterGrade':
          averageGrade > 0 ? Grade.calculateLetterGrade(averageGrade) : 'N/A',
    };
  }
