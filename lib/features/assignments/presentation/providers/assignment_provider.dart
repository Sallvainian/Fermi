...
class AssignmentProvider with ChangeNotifier {
  ...
  List<Assignment> get overdueAssignments {
    ...
    return _assignments
        .where((a) =>
            a.dueDate.isBefore(now) && a.status != AssignmentStatus.completed)
        .toList()
  ...
  Future<bool> updateAssignment(Assignment assignment) async {
    ...
    try {
      ...
      // Update local list
      final index =
          _teacherAssignments.indexWhere((a) => a.id == assignment.id);
      if (index != -1) {
  ...
  Future<bool> togglePublishStatus(String assignmentId, bool publish) async {
    ...
    try {
      if (publish) {
        final assignment =
            await _assignmentRepository.getAssignment(assignmentId);
        if (assignment != null) {
  ...
  Future<bool> updateAssignmentStatus(
      String assignmentId, AssignmentStatus status) async {
    _setLoading(true);
    try {
