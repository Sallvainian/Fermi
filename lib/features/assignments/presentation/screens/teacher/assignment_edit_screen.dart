...
class AssignmentEditScreen extends StatefulWidget {
  ...
  const AssignmentEditScreen({super.key, required this.assignmentId});
...
class _AssignmentEditScreenState extends State<AssignmentEditScreen> {
    ...
    try {
      ...
      if (assignment != null && mounted) {
        setState(() {
          ...
          _maxPointsController.text = (assignment['maxPoints'] ?? 0)
              .toInt()
              .toString();
          _selectedType = assignment['type'] ?? 'essay';
          _dueDate =
              _parseDateTime(assignment['dueDate']) ??
              DateTime.now().add(const Duration(days: 7));
          _dueTime = TimeOfDay.fromDateTime(
            _parseDateTime(assignment['dueDate']) ?? DateTime.now(),
          );
          ...
          if (assignment['publishAt'] != null) {
            ...
            _scheduledPublishTime = TimeOfDay.fromDateTime(
              _parseDateTime(assignment['publishAt']) ?? DateTime.now(),
            );
          }
  ...
  Future<void> _saveChanges() async {
    ...
    try {
      ...
      final success = await assignmentProvider.updateAssignment(
        widget.assignmentId,
        updatedData,
      );
                            ...
                            onChanged: (value) {
                              setState(() {
                                ...
                                if (!_publishNow &&
                                  ...
                                  _scheduledPublishDate = DateTime.now().add(
